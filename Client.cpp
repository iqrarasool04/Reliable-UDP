...
using namespace std;
/*
 * This class is for the implementation of the reliable UDP Client which requests the 
 * UDP Server for a file. It will send  the cumulative acknowledgement for correctly
 * received packets in the window.
 * 
 * By:  Iqra Rasool, Sheeza Aslam - CS 11B
 */
bool timeout;
class UDPClient
{
...
{
	reliableUDPData *receiveBuffer=new reliableUDPData[100000];
	...
	while(segment.ackFlag==0)
	{
		...
		while(winCount<noOfSegInWin && segment.ackFlag==0)		{
			...
			if(FD_ISSET(clientSocket,&fds) && val==1)
			{
				...
				if(seqNo==nxtExpectedSeqNum && receiveBufferInd<50000)
				{
...
{        
	...
	ofstream writeFile;
  	writeFile.open("Received_File.3gp"); //can mention any type depending on requested file for example txt, mp4 , 3gp etc
